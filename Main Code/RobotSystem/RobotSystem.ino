...
// --------- Variables ---------                                 // --------- Variables ---------
byte robot_test_state = 5;                                       // Variable to control if the robot runs a test or not. 0 = Normal, 1 = Test 1, 2 = Test 2 etc. See tests.txt for details
...
const byte LS1_PIN = 2;                                          // Pins used for Line Sensors (LS) 1, 2 and 3
const byte LS2_PIN = 1;
const byte LS3_PIN = 0;
...
int take_IR_sensor_reading(int IR_PIN=a5, int samples=107) {
...
void setup() {                                                   // Function that runs on power-up/RESET
  ...
  pinMode(trigPin, OUTPUT);                                       //Declare pins for ultrasound sensor
