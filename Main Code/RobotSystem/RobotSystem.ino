...
// --------- Variables ---------                                 // --------- Variables ---------
byte robot_test_state = 6;                                       // Variable to control if the robot runs a test or not. 0 = Normal, 1 = Test 1, 2 = Test 2 etc. See tests.txt for details
  ...
  if (tick_counter*tick_length < sweep_start_time + sweep_time) {
    ...
    if (not dummy_located) {
      ...
      if (IR_sensor_magnitude < max_IR_sensor_magnitude*threshold) {
        ...
        dummy_located = true;
        sweep_start_time = 0;
        drive_motor(left_motor_port, 0, true);                        
        drive_motor(right_motor_port, 0, true);
        delay(1000000);
